...
var serviceLocation = "ws:"+document.location.host+"/sampleHospital/chat/";
var chatBoxDetails = {};
...
function addRoom() {
   ...
   for(var i=0; i<users.length; i++){
       tonotification+='<users id="'+users[i]+'"></users>';
    }
      var toSendnotificatio='<notification>'+tonotification+'</notification>';
   ...
   and can authneticate with the password "+password +". Br Admin sandesh. ");
    ...
    var toappend = '<id></id><name></name><password></password>';
    for (var i = 0; i < users.length; i++) {
        toappend += '<users id="' + users[i] + '"></users>';
    }
    var toSend = '<Cases>' + toappend + '</Cases>';
    ...
    ajaxRoom.find('name').append(roomName);
    for (var i = 0; i < users.length; i++) {
...
function getUsers(data, status) {
    ...
    $(data).find('user').each(function () {
        ...
        $(this).find('cases').nextSibiling;
        $(this).find('notifications').nextSibiling;
        $(this).find('messages').nextSibiling;
        
      var name =$(this).find('user>name').text();
     
       var otherid = $(this).find('user>id').text();
...
function requestMessage(senderID, recieverID, name) {
    ...
    setInterval(function () {
        $.ajax({
            url: "webapi/messages?firstUserId=" + recieverID + "&secondUserId" + senderID,
            method: "GET",
            datatype: "xml",
            success: getMessage,
